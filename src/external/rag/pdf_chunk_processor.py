...
# RAG-specific paths (localized for src/external/rag/)
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
# SCRIPT_DIR is now /home/syk/projects/experiment-plan-design/src/external/rag/

# Default paths (can be overridden by command-line arguments)
DEFAULT_PDF_DIR = os.path.join(SCRIPT_DIR, 'data', 'test_papers')
DEFAULT_OUTPUT_DIR = os.path.join(SCRIPT_DIR, 'data', 'test_papers')  # JSON files saved alongside PDFs
LOG_FILE = os.path.join(SCRIPT_DIR, 'pdf_processing.log')
...
def process_single_pdf(pdf_path: str, output_dir: str, converter: DocumentConverter, chunker: HybridChunker) -> bool:
    ...
    Output format matches RAG's content_list_process.json format:
    [
        {
            "type": "text",
            "text": "chunk content",
    ...
    """
    pdf_filename = os.path.basename(pdf_path)
    # Create subdirectory for each PDF (like original literature structure)
    pdf_base_name = os.path.splitext(pdf_filename)[0]
    pdf_output_dir = os.path.join(output_dir, pdf_base_name)
    os.makedirs(pdf_output_dir, exist_ok=True)

    # Output as content_list_process.json
    output_json_path = os.path.join(pdf_output_dir, 'content_list_process.json')
    ...
    try:
        ...
        for i, chunk in enumerate(chunk_iter):
            ...
            if keep_chunk:
                # Format to match RAG's content_list_process.json structure
                chunk_data = {
                    "type": "text",
                    "text": chunk_text,
                    "text_level": 2,  # Default to level 2 (content); could enhance to detect titles
                    "page_idx": i  # Use chunk index as page_idx approximation
                }
        ...
        # 5. Save to JSON in RAG-compatible format
        # Already created pdf_output_dir earlier
        with open(output_json_path, 'w', encoding='utf-8') as f:
            json.dump(filtered_chunks, f, ensure_ascii=False, indent=2)

        # Update log message
        logger.info(f"Successfully processed {pdf_filename} -> {pdf_base_name}/content_list_process.json ({len(filtered_chunks)} chunks saved after filtering, DOI Found: {extracted_doi is not None})")
        return True
...
def main(args):
    ...
    for pdf_file in pdf_files:
        ...
        pdf_output_dir = os.path.join(args.chunks_dir, pdf_base_name)
        output_json_path = os.path.join(pdf_output_dir, 'content_list_process.json')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process PDF files for RAG indexing: chunk them and save in content_list_process.json format.",
        epilog="Example: python pdf_chunk_processor.py --input-path data/test_papers"
    )
    parser.add_argument(
        "--input-path",
        default=DEFAULT_PDF_DIR,
        help=f"Path to the input PDF file or directory containing PDF files (default: {DEFAULT_PDF_DIR})"
    )
    parser.add_argument(
        "--chunks-dir",
        default=DEFAULT_OUTPUT_DIR,
        help=f"Directory to save output JSON files (default: {DEFAULT_OUTPUT_DIR})"
    )
    parser.add_argument(
        "--overwrite",
        action='store_true',
